...
class _EmbeddingDeliveryBridge(QObject):
    ...
    progressed = Signal(object, object)  # (callback, info_dict)
...
def _delivery_bridge(parent: QObject) -> _EmbeddingDeliveryBridge:
    ...
    if bridge is None:
        ...
        bridge.progressed.connect(_handle_delivery_progress)
...
def _handle_delivery_progress(callback: Callable[[dict], None], info: dict) -> None:
    callback(info)
...
class EmbeddingJobSpec:
    job_type: str  # load | download | message_index | chunk_index | vector_search | conversational_search
    db_path: Path
    dataset_id: int
    adapter_key: str
    model_id: str
...
class _LoadOnlyLogger:
    def info(self, *args: Any, **kwargs: Any) -> None:
        return None


def _execute(spec: EmbeddingJobSpec, on_progress: Callable[[dict], None] | None = None) -> Any:
    ...
    if spec.job_type == "load":
        info = _service.load(spec.adapter_key, spec.model_id)
    ...
    if spec.job_type == "download":
        info = _service.download(spec.adapter_key, spec.model_id)
        return EmbeddingLoadResult(
            model_name=info.model_name,
            dimensions=info.dimensions,
            normalization_mode=info.normalization_mode,
    ...
    try:
        ...
        adapter, info = _service.require_loaded()
...
def _worker_loop() -> None:
    while True:
        ...
        try:
            if work.on_progress is not None:
                def _progress(info: dict) -> None:
                    _delivery_bridge(work.parent).progressed.emit(work.on_progress, info)
                result = _execute(work.spec, on_progress=_progress)
            else:
...
def download_embedding_model(
    ...
    *,
    ...
    """Queue embedding model download on the dedicated worker thread (network required, one-time)."""
    from message_evidence_workstation.config.settings import load_settings
    from message_evidence_workstation.embeddings.model_registry import get_model_spec
