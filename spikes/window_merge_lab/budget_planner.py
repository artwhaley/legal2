"""Synthesis budget planner — chooses between full and compact direct synthesis."""
...
def estimate_full_direct_output_tokens(range_count: int) -> int:
    return 1_500 + range_count * 190


def estimate_compact_direct_output_tokens(range_count: int) -> int:
...
def plan_synthesis_budget(request: SynthesisBudgetRequest) -> SynthesisBudgetPlan:
    ...
    full_output = estimate_full_direct_output_tokens(range_count)
    compact_output = estimate_compact_direct_output_tokens(range_count)
    ...
    if not input_fits:
        reasons.append(
            f"input ~{estimated_input} > available ~{available_input}"
        )
    if not full_output_fits:
        reasons.append(
            f"full output ~{full_output} > available ~{available_output}"
        )
